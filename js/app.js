//Components first, then our Vue instance/s
...
var messagesList = Vue.component("messages", {
	...
	methods : {
		getRelativeTime : function(timeStamp) {
			var now = new Date(), secondsPast = (now.getTime() - timeStamp
					.getTime()) / 1000;
			if (secondsPast < 60) {
			...
			if (secondsPast > 86400) {
				...
				month = timeStamp.toDateString().match(/ [a-zA-Z]*/)[0]
						.replace(" ", "");
...
var ChatApp = new Vue({
	el : "#root",
	data : {
		appName : "Welcome to the Vue JS Chat App",
		messages : [ 
